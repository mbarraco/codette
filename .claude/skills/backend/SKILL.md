...
The backend follows a layered architecture. Dependencies flow inward: **API → Handler → Service → Adapters → Models**.

```
app/
  api/             # API layer: routers, request/response schemas
  handlers/        # Handler layer: use-case / business logic orchestration
  services/        # Services layer: business logic coordinating adapters
  adapters/        # Adapters layer: all external boundaries
    repository/    #   DB access (one file per model)
    db.py          #   DB session management (engine, SessionLocal)
    storage.py     #   Object storage (GCS / fake-gcs)
    ...            #   Other adapters: job runners, external APIs
...
2. **Handler** (`app/handlers/`) — Orchestrates use cases. Receives validated data from the API layer, applies business rules, and calls one or more services/adapters. Returns domain models or raises domain errors.

3. **Services** (`app/services/`) — Business logic that coordinates multiple adapters. Handlers delegate to services; workers may call services directly. Services receive `db: Session` and adapter instances as arguments.

4. **Adapters** (`app/adapters/`) — All external boundaries live here. Two categories:
...
5. **Worker** (`app/worker/`) — Long-running orchestrators. Receive adapters and `db: Session` via constructor injection. Own the processing flow, not the infrastructure details.

6. **Models** (`app/models/`) — SQLAlchemy 2.0 `DeclarativeBase` models using `Mapped` / `mapped_column`. One file per table, re-export from `__init__.py`. Models serve as domain entities — intentional coupling of domain and persistence for pragmatism.
